...
def handler(event, context):
    ...
    for s3_record in s3_records:
        ...
        if bucket != STAGING_BUCKET:
            logger.warning(
                f"Non staging bucket event received. Received event from '{bucket}' bucket. Skipping... "
            )
            continue
    ...
    logger.info(
        f"Processing {len(manifest_records)}/{len(non_manifest_records)} manifest/non-manifest events."
    )
    ...
    if len(manifest_records) > 0:
        ...
        # Create folder lock payload
        for record in manifest_records:
            s3key: str = record["s3"]["object"]["key"]

            payload = {"task": "FOLDER_LOCK", "submission_prefix": s3key}

            folder_lock_response = call_lambda(FOLDER_LOCK_LAMBDA_ARN, payload)
            logger.info(f"Folder Lock Lambda call response: {folder_lock_response}")

        # Call validation lambda
        validation_response = call_lambda(
            MANIFEST_PROCESSOR_LAMBDA_ARN, {"Records": manifest_records}
        )

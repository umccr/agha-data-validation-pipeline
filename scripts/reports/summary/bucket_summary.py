...
def parse_json_with_pandas():
    f = open('agha_gdr_store_bucket.json')
    data = json.load(f)
    f.close()
...
def summarize_files_from_bucket():
    # TODO: Settings to check for only consent files or all files
    consent = True

    # Some data storage
    report_txt = Report()
    data_json = Data()
    pd_df = parse_json_with_pandas()
    ...
    for new_flagship in flagships:
        ...
        report_txt.insert_heading(f'Checking FlagShip {flagship_official_name}')
        ...
        # TODO:Add to data class and write json file for the statistic for post processing data.

        report_txt.flush()


if __name__ == '__main__':
    summarize_files_from_bucket()

...
def update_dynamodb(s3_key_list):
    ...
    for new_s3_key in s3_key_list:
        ...
        dy_res = dynamodb.get_item_from_exact_pk_and_sk(table_name=DYNAMODB_STORE_TABLE, partition_key=pk,
                                                        sort_key=new_s3_key)
    ...
    dynamodb.batch_write_objects_archive(DYNAMODB_STORE_ARCHIVE_TABLE, update_array, 'ObjectUpdate')
...
def main():
    ...
    for row in parsed_pd.itertuples():
        ...
        if agha.FileType.is_compressable_file(new_s3_key) and not agha.FileType.is_compress_file(new_s3_key):
            new_s3_key += '.gz'

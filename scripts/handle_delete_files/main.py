...
def delete_file():
    ...
    if not DRY_RUN:
        ...
        print(f'Sync results bucket for the following submissions: {json.dumps(directory_prefix, indent=4)}')
    ...
    if not DRY_RUN and FILE_BUCKET_LOCATION == STORE_BUCKET:
        ...
        print(f'Updating `manifest.txt` for the following submissions: {json.dumps(directory_prefix, indent=4)}')
    else:

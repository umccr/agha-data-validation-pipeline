...
def get_arguments():
    ...
    parser.add_argument(
        "--sort_key_prefix", required=True, type=str, help=" submission prefix"
    )
...
def sync_result_bucket(args):
    ...
    if sync_from == STAGING_BUCKET or sync_from == STORE_BUCKET:
        metadata_list_main_bucket = s3.get_s3_object_metadata(
            bucket_name=sync_from, directory_prefix=sort_key_prefix
        )
        filename_main_bucket = [
            metadata["Key"].split("/")[-1] for metadata in metadata_list_main_bucket
        ]

        # Removing index file from the main bucket
        # Index file is to be ignored as it will not have its own results/log file
        filename_main_bucket = [
            key for key in filename_main_bucket if not agha.FileType.is_index_file(key)
        ]

    elif (
        sync_from == DYNAMODB_STAGING_TABLE_NAME
        or sync_from == DYNAMODB_STORE_TABLE_NAME
    ):
        item_res = dynamodb.get_batch_item_from_pk_and_sk(
            table_name=sync_from,
    ...
    filename_main_bucket = [
        find_file_base_name(filename) for filename in filename_main_bucket
    ]
    # Grab results file data
    metadata_list_results_bucket = s3.get_s3_object_metadata(
        bucket_name=RESULT_BUCKET, directory_prefix=sort_key_prefix
    )
    s3key_results_bucket = [
        metadata["Key"] for metadata in metadata_list_results_bucket
    ]

...
def handler(event, context):
    ...
    fail_batch_key = run_batch_check(submission_directory)
...
def run_batch_check(staging_directory_prefix: str, exception_list:list=list()) -> list:
    '''
    ...
    for s3_item in s3_list:
        ...
        if agha.FileType.is_index_file(s3_key) or agha.FileType.from_name(s3_key) == agha.FileType.MANIFEST\
                or agha.FileType.is_md5_file(s3_key) or s3_key in exception_list:
            continue

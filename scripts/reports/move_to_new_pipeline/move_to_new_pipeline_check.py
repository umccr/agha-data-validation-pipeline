    ...
    for old_flagship in mapping:
        ...
        if not isinstance(val, list):
            ...
            report_message.append(
                f"Number of submission checked for {new_flagship}: {len(submission_s3_list)}"
            )
        else:
            new_flagship = val[0]
        ...
        report_message.append(
            f"Moving flagship name from {old_flagship} to {new_flagship}"
        )
        ...
        for submission_prefix in submission_s3_list:
            ...
            for metadata in old_metadata_list:
                ...
                if not move_submission == None:
                    old_store_s3_key = add_new_submission(
                        old_store_s3_key, move_submission
                    )

                # Skipping for Index and manifest file
                if (
                    agha.FileType.is_index_file(old_store_s3_key)
                    or agha.FileType.is_compressable(
                        agha.FileType.from_name(old_store_s3_key).value
                    )
                    or agha.FileType.from_name(old_store_s3_key)
                    == agha.FileType.MANIFEST
                ):
                    number_of_index_or_compress_file += 1
                    continue

                # Grab new store metadata
                new_store_s3_key = replace_old_flagship_name(
                    old_store_s3_key, new_flagship
                )

                try:
                    # Searching file in staging bucket
                    new_bucket_name = new_staging_bucket_name
                    new_metadata = get_object_metadata(
                        bucket_name=new_bucket_name, s3_key=new_store_s3_key
                    )
                    ...
                    try:
                        ...
                        report_dict = construct_mismatch_dict(
                            metadata, reason="No key found at new staging/store bucket"
                        )
                ...
                if old_store_etag != new_store_etag or old_store_size != new_store_size:
                    report_dict = construct_mismatch_dict(
                        old_pipeline_dict=metadata,
                        new_pipeline_dict=new_metadata,
                        reason="Mismatch etag and/or size value",
                    )
                    error_list.append(report_dict)

            report_message.append(
                f"Total number of file from old store: {len(old_metadata_list)}"
            )
            report_message.append(
                f"Total number of file skipped (Skipping index and/or manifest files) : {number_of_index_or_compress_file}"
            )
            report_message.append(
                f"Total number of file checked: {len(old_metadata_list)-number_of_index_or_compress_file}"
            )
        ...
        write_message_to_file(
            filename=f"{old_flagship}_new_pipeline_report_{util.get_datetimestamp()}.txt",

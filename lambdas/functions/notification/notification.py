...
def handler(event, context):
    ...
    if EMAIL_NOTIFY == "yes":
        ...
        logger.info(json.dumps(email_response, cls=util.JsonSerialEncoder))

    if SLACK_NOTIFY == "yes":
        logger.info(f"Sending notification to {SLACK_CHANNEL}")

        slack_message = "\n".join(messages)
        slack_message = slack_message.replace("&nbsp;", " ").replace("<br>", "\n")
...
def call_slack_webhook(
    topic, title, message, slack_host, slack_channel, slack_webhook_endpoint
):

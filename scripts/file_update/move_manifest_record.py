...
def get_arguments():
    ...
    parser.add_argument(
        "--old_sort_key", required=True, type=str, help="Source sort key"
    )
    parser.add_argument(
        "--new_sort_key", required=True, type=str, help="Target source key"
    )
    parser.add_argument(
        "--value_override",
        required=False,
...
def move_and_update_manifest_record(event_payload):
    ...
    dynamodb.batch_write_objects(table_name=table_name, object_list=[dynamodb_item])

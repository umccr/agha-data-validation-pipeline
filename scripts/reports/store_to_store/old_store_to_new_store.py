...
skipped_list = ["Mito/2019-04-18/", "HIDDEN/2019-09-27/"]
...
def check_etag_and_size_match():
    ...
    for old_flagship in mapping:
        ...
        report_message.append(
            f"Moving flagship name from '{old_flagship}' to '{new_flagship_name}'"
        )
        ...
        for submission_prefix in submission_s3_list:
            ...
            if move_submission is not None:
                ...
                new_submission_prefix = replace_old_flagship_name(
                    submission_prefix, new_flagship_name
                )
            ...
            manifest_data = submission_data.retrieve_manifest_data(
                bucket_name=OLD_STORE_BUCKET_NAME,
                manifest_key=f"{submission_prefix}manifest.txt",
            )

            # Grabbing old metadata for old submission
            old_metadata_list = s3.get_s3_object_metadata(
                bucket_name=OLD_STORE_BUCKET_NAME, directory_prefix=submission_prefix
            )
            # Categorizing files
            ...
            list_of_old_store_compressible_file = [
                metadata
                for metadata in list_of_old_submission_s3_keys
                if agha.FileType.is_compressable(
                    agha.FileType.from_name(metadata).get_name()
                )
            ]
            list_of_old_store_compressed_file = [
                metadata
                for metadata in list_of_old_submission_s3_keys
                if agha.FileType.is_compress_file(metadata)
            ]
            list_of_old_store_uncompress_file = list(
                set(list_of_old_store_compressible_file)
                - set(list_of_old_store_compressed_file)
            )
            list_of_old_store_indexable_file = [
                metadata
                for metadata in list_of_old_submission_s3_keys
                if agha.FileType.is_indexable(
                    agha.FileType.from_name(metadata).get_name()
                )
            ]
            list_of_old_store_indexed_file = [
                metadata
                for metadata in list_of_old_submission_s3_keys
                if agha.FileType.is_index_file(metadata)
            ]
            list_of_old_store_md5_file = [
                metadata
                for metadata in list_of_old_submission_s3_keys
                if agha.FileType.is_md5_file(metadata)
            ]

            # Create Statistic
            statistic_report_old_submission = get_filetype_statistic_from_key_array(
                list_of_old_submission_s3_keys
            )
            statistic_report_old_submission["number_of_indexed_file"] = len(
                list_of_old_store_indexed_file
            )
            statistic_report_old_submission["number_of_indexable_file"] = len(
                list_of_old_store_indexable_file
            )
            statistic_report_old_submission["number_of_uncompress_file"] = len(
                list_of_old_store_uncompress_file
            )
            statistic_report_old_submission["number_of_compressed_file"] = len(
                list_of_old_store_compressed_file
            )
            statistic_report_old_submission["number_of_files"] = len(
                list_of_old_submission_s3_keys
            )

            # Grabbing new metadata list for new submission
            new_metadata_list = s3.get_s3_object_metadata(
                bucket_name=NEW_STORE_BUCKET_NAME,
            ...
            for metadata in new_metadata_list:
                ...
                res = dynamodb.get_item_from_exact_pk_and_sk(
                    table_name="agha-gdr-store-bucket",
            ...
            list_of_new_store_compressible_file = [
                metadata
                for metadata in list_of_new_submission_s3_keys
                if agha.FileType.is_compressable(
                    agha.FileType.from_name(metadata).get_name()
                )
            ]
            list_of_new_store_compressed_file = [
                metadata
                for metadata in list_of_new_submission_s3_keys
                if agha.FileType.is_compress_file(metadata)
            ]
            list_of_new_store_uncompress_file = list(
                set(list_of_new_store_compressible_file)
                - set(list_of_new_store_compressed_file)
            )
            list_of_new_store_indexable_file = [
                metadata
                for metadata in list_of_new_submission_s3_keys
                if agha.FileType.is_indexable(
                    agha.FileType.from_name(metadata).get_name()
            ...
            list_of_new_store_indexed_file = [
                metadata
                for metadata in list_of_new_submission_s3_keys
                if agha.FileType.is_index_file(metadata)
            ]

            statistic_report_new_submission = get_filetype_statistic_from_key_array(
                list_of_new_submission_s3_keys
            )
            statistic_report_new_submission["number_of_indexed_file"] = len(
                list_of_new_store_indexed_file
            )
            statistic_report_new_submission["number_of_indexable_file"] = len(
                list_of_new_store_indexable_file
            )
            statistic_report_new_submission["number_of_uncompress_file"] = len(
                list_of_new_store_uncompress_file
            )
            statistic_report_new_submission["number_of_compressed_file"] = len(
                list_of_new_store_compressed_file
            )
            statistic_report_new_submission["number_of_files"] = len(
                list_of_new_submission_s3_keys
            )

            old_minus_new_and_etc_list = list(
                set(list_of_old_submission_s3_keys)
                - set(list_of_new_submission_s3_keys)
                - set(list_of_old_store_md5_file)
                - set(list_of_old_store_indexed_file)
                - set(list_of_old_store_uncompress_file)
            )

            new_minus_old_list = list(
                set(list_of_new_submission_s3_keys)
                - set(list_of_new_store_indexed_file)
                - set(list_of_new_store_compressible_file)
                - set(list_of_new_submission_s3_keys)
            )
            ...
            check["is_number_index_and_non_index_file_match_in_new_store"] = (
                statistic_report_new_submission["number_of_indexable_file"]
                == statistic_report_new_submission["number_of_indexed_file"]
            )

            # 2. Number of compressed + uncompressed in old store is the same as number of compressed file in the new store
            check[
                "is_number_of_compressible_file_in_old_store_is_the_same_as_number_of_compressed_file_in_new_store"
            ] = (
                len(list_of_old_store_compressible_file)
                == statistic_report_new_submission["number_of_compressed_file"]
            )

            # 3. Number of indexable file in staging and store are the same
            check["is_number_of_indexable_file_match_between_buckets"] = (
                statistic_report_old_submission["number_of_indexable_file"]
                == statistic_report_new_submission["number_of_indexable_file"]
            )
            ...
            overall_report[
                "number_of_old_bucket_minus_new_bucket_excluding_index_uncompress_md5"
            ] = len(old_minus_new_and_etc_list)
            overall_report["number_of_extra_files_in_new_bucket"] = len(
                new_minus_old_list
            )
            overall_report[
                "files_in_old_bucket_minus_new_bucket_excluding_index_uncompress_md5"
            ] = old_minus_new_and_etc_list
            ...
            if len(old_minus_new_and_etc_list) > 0 or [
                result for result in validation_value_results_list if result is True
            ]:
                attention_dict = {
            ...
            report_message.append(
                json.dumps(overall_report, indent=4, cls=util.JsonSerialEncoder)
            )
    ...
    write_message_to_file(
        filename=f"move_store_to_store_report.json",
        message_list=[json.dumps(move_output, indent=4)],
    )

    # Append some filename needs attention
    report_message.append("\n Please double check the following array: \n\n")
    report_message.append(
        json.dumps(attention_list, indent=4, cls=util.JsonSerialEncoder)
    )

    # Write result to json
    write_message_to_file(
        filename=f"old_store_to_new_store_pipeline_report_{util.get_datetimestamp()}.txt",
...
def get_filetype_statistic_from_key_array(s3_key_list=None):
    ...
    for s3_key in s3_key_list:
        ...
        ):  # Make some annotation what is the unsupported file
...
def replace_old_flagship_name(
    key_with_old_flagship: str, new_flagship_name: str
) -> str:

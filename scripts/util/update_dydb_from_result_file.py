...
def update_dydb_from_result_file():
    ...
    s3_metadata_list = s3.get_s3_object_metadata(
        bucket_name=RESULTS_BUCKET_NAME, directory_prefix=directory_prefix
    )
    ...
    for metadata in s3_metadata_list:
        ...
        result_list = s3.get_object_from_bucket_name_and_s3_key(
            s3_key=s3_staging_key, bucket_name=RESULTS_BUCKET_NAME
        )
    ...
    dynamodb.batch_write_records(
        DYNAMODB_ARCHIVE_RESULT_TABLE_NAME, dynamodb_archive_put_item_list
    )
...
def get_checksum_from_manifest_record(table_name, staging_s3_key):
    record_response = dynamodb.get_item_from_pk_and_sk(
...
def validate_batch_job_result(batch_result: dict):
    ...
    if type == batch.Tasks.CHECKSUM_VALIDATION.value:
        ...
        provided_checksum = get_checksum_from_manifest_record(
            DYNAMODB_STAGING_TABLE_NAME, s3_key
        )

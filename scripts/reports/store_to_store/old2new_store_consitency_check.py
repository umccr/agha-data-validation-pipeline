...
def check_all():
    validation_report.add_msg(f'Comparison Report between old/new store ({BUCKET_NAME_STORE_OLD} / {BUCKET_NAME_STORE})')
    ...
    for root in roots:
        if is_legacy_sub(root):
            ...
            continue
        all_prefixes.extend(s3.aws_s3_ls(bucket_name=BUCKET_NAME_STORE_OLD, prefix=root))
    validation_report.add_msg(f"Total number of prefixes (submissions): {len(all_prefixes)}", 1)
    ...
    for fs in prefix_by_fs_map.keys():
        ...
        for prefix in prefix_by_fs_map[fs]:
            ...
            if exists_submission(bucket=BUCKET_NAME_STORE, prefix=prefix_new):
                validation_report.add_msg(f"Checking {BUCKET_NAME_STORE_OLD}/{prefix} against {BUCKET_NAME_STORE}/{prefix_new}", 1)
                compare_s3_listing(bucket_1=BUCKET_NAME_STORE_OLD, prefix_1=prefix,
                                   bucket_2=BUCKET_NAME_STORE, prefix_2=prefix_new)
            else:
                validation_report.add_msg(f"WARNING: could not find new submission for {BUCKET_NAME_STORE_OLD}/{prefix}", 1)
...
def compare_s3_listing(bucket_1, prefix_1, bucket_2, prefix_2):
    ...
    s3_list_1 = s3.get_s3_object_metadata(bucket_name=bucket_1, directory_prefix=prefix_1)
    s3_list_2 = s3.get_s3_object_metadata(bucket_name=bucket_2, directory_prefix=prefix_2)
    ...
    validation_report.add_msg(f"Number of records only in {bucket_1}: {df_1_only.shape[0]}", 2)
    df_2_only = diff_df.loc[diff_df['_merge'] == 'right_only']
    validation_report.add_msg(f"Number of records only in {bucket_2}: {df_2_only.shape[0]}", 2)
    ...
    for index, row in df_1_only.iterrows():
        ...
        if agha.FileType.is_index_file(row.Key):
            ...
            key_1_list_explained_cnt += 1
            continue
        if not agha.FileType.is_compress_file(row.Key) and agha.FileType.is_compressable_file(row.Key):
    ...
    validation_report.add_msg(f"{key_1_list_explained_cnt}/{df_1_only.shape[0]} differences explainable ({df_1_only_issues.shape[0]} unexplained).", 3)
    if df_1_only_issues.shape[0] > 0:
        validation_report.add_msg(f"Unexplained differences:", 3)
        validation_report.add_msg(f"--------------------------------------------------------------------------------", 3)
    ...
    for index, row in df_2_only.iterrows():
        ...
        if agha.FileType.is_index_file(row.Key):
            ...
            key_2_list_explained_cnt += 1
            continue
        if agha.FileType.is_compress_file(row.Key) and agha.FileType.is_compressable_file(row.Key):
            # we automatically compress, so there should be a compressed equivalent
    ...
    validation_report.add_msg(f"{key_2_list_explained_cnt}/{df_2_only.shape[0]} differences explainable ({df_2_only_issues.shape[0]} unexplained).", 3)

...
class SubmissionData:
    def __init__(self, bucket_name="", manifest_s3_key="", submission_prefix=""):
        # Event data
    ...
    def create_submission_data_object_from_s3_event(cls, s3_record):
        ...
        return cls(bucket_name=bucket, manifest_s3_key=s3_key, submission_prefix=prefix)


def retrieve_manifest_data(bucket_name: str, manifest_key: str):
    client_s3 = util.get_client("s3")

    logger.info(f"Getting manifest from: {bucket_name}/{manifest_key}")

    try:
        manifest_obj = client_s3.get_object(Bucket=bucket_name, Key=manifest_key)
...
def validate_manifest(
    data: SubmissionData,
    postfix_exception_list: list,
    skip_checksum_check: bool = False,
):
    ...
    # Entry count
    notification.log_and_store_message(
        f"Entries in manifest: {len(data.manifest_data)}"
    )
    ...
    for missing_filename in files_missing_from_manifest:
        if is_file_skipped(missing_filename, postfix_exception_list):
            ...
            logger.info(
                f"{missing_filename} is in the exclusion filetype list. "
    ...
    if files_missing_from_manifest:
        ...
        notification.log_and_store_message(
            "Submission contain file that is not in the manifest. "
            "Proceeding with only file in the manifest.",
            level="critical",
        )
        notification.log_and_store_message(
            "Files not in the manifest will NOT be store at GDR store bucket",
            level="critical",
        )
    ...
    for row in data.manifest_data.itertuples():
        ...
        # Study ID
        agha_study_id_list = row.agha_study_id.split(
            ","
        )  # Separate by commas for multi study-id
        for agha_study_id in agha_study_id_list:
            if not AGHA_ID_RE.match(agha_study_id) and not MM_ID_RE.match(
                agha_study_id
            ):
...
def is_file_skipped(filename: str, postfix_exception_list: list = None) -> bool:
    ...
    if (
        agha.FileType.is_manifest_file(filename)
        or agha.FileType.is_index_file(filename)
        or agha.FileType.is_md5_file(filename)
        or len(
            [
                filename

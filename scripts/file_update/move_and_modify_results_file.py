...
def get_arguments():
    ...
    parser.add_argument(
        "--source_s3_key", required=True, type=str, help="Source results s3 key"
    )
    parser.add_argument(
        "--target_s3_key", required=True, type=str, help="Target results s3 key"
    )
...
def mode_and_modify_result(args):
    ...
    try:
        results_list = s3.get_object_from_bucket_name_and_s3_key(
            bucket_name=RESULT_BUCKET, s3_key=results_source_s3_key
        )

    except s3.S3_CLIENT.exceptions.NoSuchKey as e:
        print(f"File not found error: {e}")
        print(
            "Possible data has not gone through the validation or no data generated for this s3_key."
        )
        print("Exiting here ... ")
        sys.exit(1)
    ...
    print(
        f"New results.json file: {json.dumps(results_list, indent=4, cls=util.JsonSerialEncoder)}"
    )
    ...
    s3.delete_s3_object_from_key(
        bucket_name=RESULT_BUCKET, key_list=[results_source_s3_key]
    )
